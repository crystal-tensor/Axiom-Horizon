...
Current state: **stage 1w evidence hardening; B7 now has a template-priority gate showing 0 one-angle template routes clear the `gcm_h6` threshold, B2 remains a hardware-like leakage negative boundary, B5 has a canonical-environment smoke gate with 0 passed rows, and B10 remains a denominator-boundary track**.
...
| B7 | Fault-tolerance co-design | B1/B2 bridge plus FT synthesis ledger; `w8_21` claim-boundary closure with 43480 optimizer runs, 0 exact replacements, and 0 ledger removal; template-priority gate over 12 retained templates showing 0 one-angle single-template routes clear `gcm_h6`, while best `w8_21` needs at least 2 arbitrary removals per occurrence. | Improve the B7 minimum row through B1 T-resource work, or produce a symbolic KAK/Clifford-scaffold proof / alternate occurrence-removing rewrite for `gcm_h6` that removes enough arbitrary rotations per occurrence. | Demote if B1/B2-linked reductions collapse once physical layout, factory throughput, feed-forward, and occurrence-level synthesis certificates are explicit. |
