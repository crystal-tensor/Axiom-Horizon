...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; T-B2-003 leakage-flagged erasure analytic proxy now runs 480 configurations with 42 target-volume improved rows, 33 distance-5/7 improved rows, 19 high-efficiency distance-5/7 rows, max reduction 23.904x, mean reduction 4.837x, no reduced rounds, no d=3 candidates, and validation errors 0. | Replace the analytic leakage-flagged erasure proxy with a circuit-level leakage/erasure decoder or calibrated leakage model that preserves distance-5/7 target-volume reductions under noise mismatch. | Demote any B2 candidate that only reproduces the closed distance-3 aggressive reduced-round artifact or loses the d=5/d=7 leakage-erasure proxy gain under circuit-level leakage stress. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 reduced-round is closed and T-B2-003 has a leakage-flagged erasure proxy, so convert it into a circuit-level leakage/erasure decoder stress test; pair B4 and B8 into one verification lane. |
