...
Current state: **stage 1q evidence hardening; B2 now has a decoder-input contract feasibility gate, B5 has a canonical DMRG readiness gate, B6 has a structural/electronic proxy boundary, and B10 remains a denominator-boundary track**.
...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; leakage-erasure proxy/stress rows; posterior calibration; posterior-weighted risk ledger; and T-B2-008 decoder-input contract gate. The gate finds 4/10 decoder inputs available, 6 missing, 4/9 gates passed, 5 critical gates failed, strict high-purity survivors 0, and all-profile robustness false. | Persist per-shot syndrome and flag traces, inject posterior flag probabilities into a real circuit-level decoder, or collect calibrated leakage/flag data; otherwise keep the heralded-erasure route demoted. | Keep the leakage-erasure route demoted if per-shot decoder inputs or calibrated leakage/flag data remain unavailable, or if strict high-purity/all-profile survival remains zero/false. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 T-B2-008 closes the current posterior-risk route as a decoder-input contract failure, so the next gate is per-shot posterior decoder traces or calibrated leakage/flag data; pair B4 and B8 into one verification lane. |
