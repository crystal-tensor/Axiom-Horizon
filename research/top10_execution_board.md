...
Current state: **stage 1r evidence hardening; B2 now has a per-shot decoder trace packet but still lacks posterior likelihood injection, B5 has a canonical DMRG readiness gate, B6 has a structural/electronic proxy boundary, and B10 remains a denominator-boundary track**.
...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; leakage-erasure proxy/stress rows; posterior calibration; posterior-weighted risk ledger; T-B2-008 decoder-input contract gate; and T-B2-009a per-shot trace packet. The trace packet persists 3 strict challenge rows, 576 detector-bitstring traces, observable/prediction rows, and 482 synthetic detector/tick flag events, but posterior likelihood injection and calibrated flag data remain missing. | Use the persisted per-shot traces in a posterior-likelihood PyMatching/Stim decoder, or replace synthetic flag events with calibrated leakage/flag data; otherwise keep the heralded-erasure route demoted. | Keep the leakage-erasure route demoted if posterior injection or calibrated leakage/flag data remain unavailable, or if strict high-purity/all-profile survival remains zero/false. |
...
| B5 | Strongly correlated matter | Same 9 B10 D5 Hubbard rows now have oracle-tuned and non-oracle embedding denominators, exact-state-seeded MPS pressure, one-site MPS/ALS, two-site finite-DMRG-style pressure, and T-B5-005 canonical DMRG readiness gate. The readiness gate passes 0/8 production conditions, fails fixed-sector norm and canonical-environment gates, keeps seeded MPS pressure as strongest, and records 0 non-seeded rows beating seeded pressure. | Implement canonical-environment production DMRG/MPS with convergence and cost ledgers, or compare a fully costed quantum impurity/response kernel against the full D5 denominator ladder. | Demote any positive B5 route if production DMRG/MPS or equivalent tensor denominator remains stronger under the same access and response contract. |
...
| B10 | Boundary of BQP | B10-T1 now includes T-B10-012 same-access B5 bridge, T-B10-013 response-sampler cost stress, and T-B5-005 canonical DMRG readiness gate. The B5 gate passes 0/8 readiness conditions; there is still no production DMRG, no sampling oracle, no same-access positive route, no quantum advantage, and no BQP separation. B10-T2 still has the trained-spoofer/proof-gate/transcript/device-noise/Qiskit-Aer/backend-calibrated-style bridge stack. | T-B10-014 must replace the readiness/cost negative boundary with mature production DMRG/MPS or a real same-access response oracle with state-preparation, mixing, measurement, confidence, and optimizer-loop costs; separately, replace the GenericBackendV2 bridge with real backend properties or hardware verifier execution. | Keep B10-T1 negative unless a same-access sampler or production DMRG/quantum response comparison beats the full denominator ladder without hidden access advantages. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 T-B2-009a now supplies per-shot detector traces, so the next B2 gate is posterior-likelihood decoder injection or calibrated leakage/flag data; pair B4 and B8 into one verification lane. |
...
| 3 | Keep B3 demoted unless a rescue mechanism appears; advance B5 from readiness-failed prototype tensor references to mature canonical DMRG/MPS evidence or quantum response-kernel cost accounting; advance B6 from structural/electronic proxy boundary to real crystallographic/DFT/B5-computed observables plus expanded post-2008 negatives. |
