...
Current state: **stage 1t evidence hardening; B2 now has a DEM-informed detector-to-edge semantics gate but it remains a negative boundary, B5 has a canonical DMRG readiness gate, B6 has a structural/electronic proxy boundary, and B10 remains a denominator-boundary track**.
...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; leakage-erasure proxy/stress rows; posterior calibration; posterior-weighted risk ledger; T-B2-008 decoder-input contract gate; T-B2-009a per-shot trace packet; T-B2-009b posterior-likelihood injection gate; and T-B2-009c DEM-informed detector-to-edge semantics gate. The DEM gate consumes 3 strict challenge rows across 3 semantic profiles / 1728 profile-shots; best conservative profile failure delta is 0 with 0 changed predictions, while aggressive DEM semantics introduces 1 failure, so improvement gate fails and route demotion remains recommended. | Replace synthetic flag events with calibrated leakage/flag data or a hardware-like leakage model, then rerun posterior-likelihood decoding and require improvement plus all-challenge non-regression. | Keep the leakage-erasure route demoted if calibrated posterior injection still fails to improve strict challenge rows without introduced failures, or if strict high-purity/all-profile survival remains zero/false. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 T-B2-009c now closes as a negative DEM semantics boundary, so the next B2 gate is calibrated leakage/flag data or a hardware-like leakage model; pair B4 and B8 into one verification lane. |
