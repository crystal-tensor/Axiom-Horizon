...
Current state: **stage 1s evidence hardening; B2 now has a posterior-likelihood injection interface but it is a negative boundary, B5 has a canonical DMRG readiness gate, B6 has a structural/electronic proxy boundary, and B10 remains a denominator-boundary track**.
...
| B2 | Low-overhead QEC | Stim baseline plus closed reduced-round artifact boundary; leakage-erasure proxy/stress rows; posterior calibration; posterior-weighted risk ledger; T-B2-008 decoder-input contract gate; T-B2-009a per-shot trace packet; and T-B2-009b posterior-likelihood injection gate. The injection gate consumes 3 strict challenge rows across 3 profiles / 1728 profile-shots; best profile failure delta is 0 with 0 changed predictions, while strong weight shift introduces 2 failures, so improvement gate fails and route demotion remains recommended. | Replace synthetic flag events with calibrated leakage/flag data or detector-to-edge posterior semantics, then rerun posterior-likelihood decoding and require improvement plus all-challenge non-regression. | Keep the leakage-erasure route demoted if calibrated posterior injection still fails to improve strict challenge rows without introduced failures, or if strict high-purity/all-profile survival remains zero/false. |
...
| Week | Focus |
|---:|---|
| 1 | Freeze B1 proof-log report outline; B2 T-B2-009b now consumes per-shot traces but closes as a negative synthetic-injection boundary, so the next B2 gate is calibrated detector-to-edge posterior semantics or real leakage/flag data; pair B4 and B8 into one verification lane. |
