...
| T-B2-009d | B2 | open | unassigned | Builder/Baseline Adversary | Replace synthetic detector/tick flag events with calibrated leakage/flag observations or a hardware-like leakage model, then rerun posterior-likelihood decoding; require improvement plus all-challenge non-regression or keep the route demoted. |
...
| T-B5-006 | B5/B10 | open | unassigned | Builder/Baseline Adversary | Implement mature canonical-environment DMRG/MPS for the same 9 B5/B10 Hubbard response rows with stored left/right environments, orthonormal residuals, sweep convergence, no exact-state seeding, and full cost accounting, or provide a fully costed quantum response kernel that beats the denominator ladder. |
